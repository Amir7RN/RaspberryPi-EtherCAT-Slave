...
static void ESC_read_pram (uint16_t address, void *buf, uint16_t len)
{
   ...
   bcm2835_spi_write_32(ESC_PRAM_RD_ADDR_LEN_REG, (uint32_t)(ESC_PRAM_SIZE(len) | ESC_PRAM_ADDR(address)));
   ...
   while (len > 0)
   {
      ...
      bcm2835_spi_transfern((char *)buffer, (unsigned int)size);

      i = 3;
      while (fifo_cnt > 0 && len > 0)
      {
         value = (uint32_t)(buffer[i] | (buffer[i+1] << 8) | (buffer[i+2] << 16) | (buffer[i+3] << 24));
...
static void ESC_write_pram (uint16_t address, void *buf, uint16_t len)
{
   ...
   bcm2835_spi_write_32(ESC_PRAM_WR_ADDR_LEN_REG, (uint32_t)(ESC_PRAM_SIZE(len) | ESC_PRAM_ADDR(address)));
