...
int16_t generate_random_negative_value(void)
{

   return (int16_t)(rand() % 201 - 100);
}

int8_t received_data = 0; // Global variable
...
void cb_transmit(void)
{
   ...
   int16_t received_value = (int16_t)(received_data & 0xFF); // Convert the received data to signed value
